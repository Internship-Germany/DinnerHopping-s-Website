...
const helpers = (typeof window !== 'undefined' && window.dhAdminDashboard && window.dhAdminDashboard.helpers) || {};

function showDialogAlert(message, options){
  if (typeof helpers.showDialogAlert === 'function'){
    return helpers.showDialogAlert(message, Object.assign({ tone: 'error', title: 'Action error' }, options || {}));
  }
  const dlg = (window.dh && window.dh.dialog) || null;
...
function getApiFetch() {
  const candidate = helpers.apiFetch || (window.dh && window.dh.apiFetch) || window.apiFetch;
